                ...
                <table id="datatable" class="table table-striped table-bordered basic-datatables">
                    <thead style="background-color: #1E3135; color: white;">
                      <tr>
                        ...
                        <th style="color: white;" class="text-center text-uppercase text-xxs font-weight-bolder opacity-7">Nomor DO</th>
                ...
                <div class="modal-body">
                    ...
                    <div class="input-group mb-3">
                        <div class="input-group-prepend">
                          <span class="input-group-text" style="width: 120px; height: 35px; background-color: rgb(222, 222, 222);">Nomor DO</span>
                        </div>
                        <input type="text" name="nomor_do" id="nomor_do" class="form-control" style="border: 1px solid black;" readonly>
                    </div>
...
window.defaultUrl = "{{ url('/delivery_order/') }}/";
...
function cetakCV() {
    ...
    window.open(defaultUrl + "cetakCV?id_do=" + selected.id_do);
}

function cetakPT() {
    let selected = table.row('.selected').data();

    window.open(defaultUrl + "cetakPT?id_do=" + selected.id_do);
}
...
function viewDatatable() {
    table = $(".basic-datatables").DataTable({
        ajax: {
            url: "{{ route('delivery_order/datatable') }}",
            "type": "post",
            "data": function (d) {
...
function generateNomorPR() {
    $.ajax({
        url: '{{ route("delivery_order.generate_nomor_do") }}',
        type: 'GET',
        dataType: 'json',
        success: function(response) {
            if (response.success) {
                $('#nomor_do').val(response.nomor_do);
            } else {
                Swal.fire({
                    title: 'Error',
...
function collectionS2Search() {
    $('select[name=cmb_sales]').select2({
        ...
        placeholder: '',
        ajax: {
            url: "{{ url('/delivery_order/getSales') }}",
            dataType: 'json',
