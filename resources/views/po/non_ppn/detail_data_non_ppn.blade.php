                ...
                <table id="datatable" class="table table-striped table-bordered basic-datatables">
                  ...
                  <div class="row mt-3" style="margin-right: 100px;">
                    <div class="col-12">
                      <div class="card">
                        <div class="card-body">
                            <form id="form-update-ppn" method="POST" action="{{ url('/non_ppn/updateTotalPpn') }}">
                                @csrf
                                <input type="hidden" name="id_non_ppn" value="{{ $id_non_ppn ?? '' }}">
                                <div class="row">
                                    <div class="col-md-4">
                    ...
                    <div class="col-12">
                      <div class="card">
                        <div class="card-body">
                            ...
                            <form id="form_update_validasi_payment" method="POST" action="{{ url('/non_ppn/updateValidasiPayment') }}">
                                @csrf
                                <input type="hidden" name="id_non_ppn" value="{{ $id_non_ppn ?? '' }}">
                                <div class="row">
                                    <div class="col-6">
                  ...
                  <div class="col-12">
                    <div class="card">
                        <div class="card-body">
                            ...
                            <form id="form_update_incentive" method="POST" action="{{ url('/non_ppn/updateincentive') }}">
                                @csrf
                                <input type="hidden" name="id_non_ppn" value="{{ $id_non_ppn ?? '' }}">
                ...
                <div class="modal-body">
                    ...
                    <div class="row">
                        <div class="col-4">
                            <div class="input-group mb-3">
                                <div class="input-group-prepend">
                                  <span class="input-group-text" style="width: 100px; height: 35px; background-color: rgb(222, 222, 222);">QTY</span>
                                </div>
                                <input type="number" name="qty" id="qty" class="form-control pl-2" style="border: 1px solid black;">
                            </div>
                        </div>
                        ...
                        <div class="col-4">
                            <div class="input-group mb-3">
                                ...
                                <input type="text" name="unit_price" id="Unit_price" class="form-control pl-2" style="border: 1px solid black;">
                            </div>
                        </div>
                        <div class="col-4">
                            <div class="input-group mb-3">
                                <div class="input-group-prepend">
                                    <span class="input-group-text" style=" height: 35px; background-color: rgb(222, 222, 222);">Vendor</span>
                                </div>
                                <input type="text" name="vendor" id="vendor" class="form-control pl-2" style="border: 1px solid black;">
                            </div>
                        </div>
                    </div>
...
$(document).ready(function() {
    ...
    modal.find("form").on("submit", function(ev) {
        ...
        $.ajax({
            url: url,
            type: 'POST',
            data: $(this).serialize() + "&id_non_ppn=" + "<?php echo $id_non_ppn ?>",
            dataType: 'json',
            success: function(response) {
                ...
                Swal.fire({
                    ...
                    if (result.isConfirmed) {
                        ...
                        // Tutup modal dengan cara yang lebih robust
...
$('#formCogs').on('show.bs.modal', function () {
    var id_non_ppn = "{{ $id_non_ppn ?? '' }}";
    console.log('id_non_ppn:', id_non_ppn); // Debug: cek nilai id_non_ppn
    $.ajax({
        url: '/non_ppn/total_cogs',
        type: 'POST',
        data: { id_non_ppn: id_non_ppn, _token: $('input[name="_token"]').val() },
...
function viewDatatable() {
    tableDetail = $(".basic-datatables").DataTable({
        ajax: {
            url: "{{ route('non_ppn/datatabledetail') }}",
            "type": "post",
            data: {
                id_non_ppn: "<?php echo $id_non_ppn ?>"
            },
            headers: {
                        'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
            },
...
function viewDatatableCogs() {
    tableCogs = $("#datatable-cogs").DataTable({
        ajax: {
            url: "{{ route('non_ppn/datatablecogs') }}",
            "type": "post",
            data: {
                id_non_ppn: "<?php echo $id_non_ppn ?>"
            },
...
function updateSubtotalPoCostCV() {
    ...
    updateSubtotalCost(); // <-- Tambahkan ini
}
...
function updateTotalVat() {
    let subtotalPrice = $('#subtotal-price').val();
    console.log('subtotalPrice:', subtotalPrice); // Debug
    $('#total-vat').val(subtotalPrice);
}

// Pastikan fungsi ini dipanggil setiap kali tabel di-draw
tableDetail.on('draw', function() {
    // ...fungsi update lain...
    updateTotalVat();
    // ...fungsi update lain...
...
$('#subtotal-price').on('input', function() {
    updateTotalVat();
});
