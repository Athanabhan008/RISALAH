...
class ReportLaundryController extends Controller
{
    protected $fpdf;

    public function index()
    {
        return view ('report.laundry', [
            "active" => 'report'
        ]);
    }
    ...
    public function createPdfRekapLaundryPerBulan(Request $request)
    {
        $periode = str_replace('-', '', $request->periode);
        $tahun = substr($periode, 0, 4);
        $bulan = substr($periode, 4, 6);

        $data_result = DB::select("CALL sp_lap_laundry_penjualanperbulan($periode)");
        ...
        $table = new easyTables($this->fpdf, "{5, 15, 8, 8, 12}", 'border:1;font-size:9;');
		...
		$table->easyCell('BERAT', 'valign:M;align:C;');
        ...
        foreach ($data_result as $value) {
            ...
            $table->easyCell('Rp'  . number_format($value->harga,  0, ',', '.'), 'valign:M;align:L;');
            $table->easyCell($value->berat, 'valign:M;align:L;');
            $table->easyCell('Rp ' . number_format($value->total_harga, 0, ',', '.'), 'valign:M;align:R;');            $table->printRow();
        }

        $table->rowStyle('font-style:B');
		$table->easyCell('TOTAL', 'colspan:4;valign:M;align:C;');
        $table->easyCell('Rp ' . number_format(array_sum(array_column($data_result, 'total_harga')), 0, ',', '.'), 'valign:M;align:R;');
		$table->printRow();
