...
use App\Models\VwcogsNonppn;
use App\Models\VwNonppndetail;
...
class NonppnController extends Controller
{
    // public function index()
    // {
    //     $user = auth()->user();
    //     if ($user->id_role == 1) {
    //         $non_ppn = DB::select("CALL sp_data_non_ppn(0)");
    //     } else {

    //         $non_ppn = DB::select("CALL sp_data_non_ppn($user->id)");
    //     }


    //     return view('non_ppn.index',[
    ...
    public function create(Request $request)
    {
        ...
        try {
            ...
            $non_ppn = new Wapu();
            $non_ppn->id_sales = $user->id;
            $non_ppn->nama_projek = $request->nama_projek;
            $non_ppn->nomor_pr = $request->nomor_pr;
            $non_ppn->updated_at = null;
            $non_ppn->save();
    ...
    public function update(Request $request, $id)
    {
        try {
            ...
            $request->validate([
                'nama_projek' => 'required|string|max:255',
                'nomor_pr' => 'required|string|unique:non_ppn,nomor_pr,' . $id
            ], [
                'nama_projek.required' => 'Nama projek wajib diisi',
                'nomor_pr.required' => 'Nomor PR wajib diisi',
                'nomor_pr.unique' => 'Nomor PR sudah digunakan'
    ...
    public function generateNomorPr()
    {
        ...
        $lastPrNonPpn = Swasta::where('id_sales', $idSales)
            ->orderByDesc('id')
            ->first();
    ...
    {
        $request->validate([
            'id_projek' => 'required|exists:non_ppn,id',
    ...
    {
        $request->validate([
            'id_projek' => 'required|exists:non_p,id',
            'total_po_ppn' => 'required',
            'total_cost_ppn' => 'required',
            'total_margin_ppn' => 'required',
        ]);
